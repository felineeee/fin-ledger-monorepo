...
export class CaptureService {
  ...
  async captureCash(id: string) {
    return this.db.transaction().execute(async (trx) => {
      ...
      if (payment.status !== 'PENDING') throw new ConflictException(`Cannot capture cash. Payment is in ${payment.status} state.`);
      if (payment.channel !== 'IN_PERSON') throw new ConflictException('This endpoint only supports IN_PERSON channel payments.');

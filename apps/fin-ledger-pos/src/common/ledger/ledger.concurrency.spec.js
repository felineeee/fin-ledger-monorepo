...
describe('Ledger Engine Adversarial Concurrency Test', () => {
    ...
    beforeAll(async () => {
        ...
        try {
            ...
            const aliceRes = await client.query(`INSERT INTO account_snapshots (user_id, balance, currency) VALUES ($1, 10000, 'USD') RETURNING id;`, [aliceUserId]);
            aliceAccountId = aliceRes.rows[0].id;
            const bobRes = await client.query(`INSERT INTO account_snapshots (user_id, balance, currency) VALUES ($1, 0, 'USD') RETURNING id;`, [bobUserId]);
    ...
    it('should process exactly 5 transfers successfully and reject 5 due to row-level balance locks', async () => {
        ...
        for (let i = 0; i < totalThreads; i++) {
            executionPromises.push(ledgerService.executeTransfer(aliceUserId, bobAccountId, aliceAccountId, transferAmount, `Adversarial Parallel Stress Thread Block #${i}`));
        }
        const outcomes = await Promise.allSettled(executionPromises);
        const successfullTransfers = outcomes.filter((o) => o.status === 'fulfilled');
        ...
        const client = await pool.connect();
        try {
            const aliceSnapshot = await client.query('SELECT balance FROM account_snapshots WHERE id = $1', [aliceAccountId]);
            const bobSnapshot = await client.query('SELECT balance from account_snapshots WHERE id = $1', [bobAccountId]);
            ...
            const ledgerCount = await client.query('SELECT COUNT(*) FROM ledger_entries;');
